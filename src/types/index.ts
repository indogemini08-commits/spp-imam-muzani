// Types for Aplikasi SPP Sekolah
...
export interface SPPType {
  id: string;
  name: string; // e.g. "SPP Boarding", "SPP Reguler", "SPP Tahfidz"
  description: string;
  monthly_amount: number;
  period_type: 'monthly' | 'custom';
  active_months: string[]; // ["Juli", "Agustus", ...]
...
export interface EskulType {
  id: string;
  name: string; // e.g. "Futsal", "Karate", "Renang"
  description: string;
  amount: number;
...
export interface AnnualBillType {
  id: string;
  name: string; // e.g. "Daftar Ulang", "Uang Pangkal", "Seragam", "Buku"
  description: string;
  amount: number;
...
export interface AnnualBillPackage {
  id: string;
  name: string; // e.g. "Paket Daftar Ulang T.A. 2026/2027"
  academic_year_id: string;
  total_amount: number;
...
export interface Student {
  ...
  birth_date: string;
  level: 'SMP' | 'SMA' | 'SD';
  class_name: string; // e.g. "7A", "8B", "10 IPA"
  academic_year_id: string;
...
export interface Bill {
  ...
  category_id?: string;
  bill_name: string; // e.g. "SPP Juli 2026", "Eskul Futsal Agustus 2026", "Daftar Ulang"
  period_month?: string; // "Juli", "Agustus", etc.
  period_year?: number;
  amount: number;
  paid_amount: number;
...
export interface AuditLog {
  ...
  user_id: string;
  user_name: string;
  activity: string; // e.g. "Input Pembayaran", "Tambah Santri", "Approve Bukti Bayar"
  details: string;
  ip_address?: string;
  timestamp: string;
}
