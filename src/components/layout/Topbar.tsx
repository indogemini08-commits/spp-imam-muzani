...
export const Topbar: React.FC<TopbarProps> = ({
  ...
  const [syncState, setSyncState] = useState<{ status: string; lastSyncTimestamp: string | null }>({
    status: 'connected',
  ...
  useEffect(() => {
    ...
    const checkSync = async () => {
      try {
        const data = await api.system.getSyncStatus();
        if (mounted && data) {
          setSyncState({ status: data.status, lastSyncTimestamp: data.lastSyncTimestamp });
        }
      } catch (_) {}
    };
    checkSync();
    const interval = setInterval(checkSync, 25000);
    return () => {
      mounted = false;
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      const data = await api.system.getSyncStatus();
      setSyncState({ status: data.status, lastSyncTimestamp: data.lastSyncTimestamp });
      success('Database cloud realtime berhasil disinkronkan!');
    } catch (e: any) {
    ...
    <header className="sticky top-0 z-30 bg-white/95 dark:bg-[#0b1329]/95 backdrop-blur-xl border-b border-slate-200/90 dark:border-slate-800 px-4 sm:px-6 py-3.5 transition-all">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {/* Cloud Realtime Sync Status & Manual Sync Button */}
          <button
            type="button"
            ...
            title={
              syncState.lastSyncTimestamp
                ? `Sinkronisasi Cloud Realtime: Terhubung (Terakhir: ${new Date(syncState.lastSyncTimestamp).toLocaleTimeString('id-ID')}). Klik untuk sinkronkan sekarang.`
                : 'Sinkronisasi Cloud Realtime: Terhubung. Klik untuk sinkronkan data sekarang.'
            }
            className="flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-50/80 hover:bg-emerald-100/90 dark:bg-emerald-950/40 dark:hover:bg-emerald-900/50 border border-emerald-200/90 dark:border-emerald-800/70 rounded-xl text-xs font-bold text-emerald-800 dark:text-emerald-300 transition-all cursor-pointer shadow-xs active:scale-95"
          >
            <div className="relative flex items-center justify-center">
              <Cloud className={`w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 ${isManualSyncing ? 'animate-bounce' : ''}`} />
              <span className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" />
              <span className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-emerald-500" />
            </div>
            <span className="hidden md:inline">
              {isManualSyncing ? 'Sinkron...' : 'Realtime'}
            </span>
            <RefreshCw className={`w-3 h-3 text-emerald-600 dark:text-emerald-400 ${isManualSyncing ? 'animate-spin' : ''}`} />
          </button>
